...
function MobileDashboard({ onLogout }) {
  ...
  async function handleSearch() {
    ...
    try {
      ...
      const routeResult = await getRoute(startMarker, destCoords);
      setActiveRoute(routeResult.coordinates);
      setDestMarker(destCoords);
      setRouteInfo({
        from: "Dubai Marina",
        to: searchText,
        time: `${routeResult.estimated_time} min`
      });
    } catch (err) {
      ...
      <section className="phone-shell">
        ...
        <section className="map-panel">
          ...
          >
            ...
            >
              <Popup>Start: {routeInfo.from}</Popup>
            </CircleMarker>
            ...
            >
              <Popup>Destination: {routeInfo.to}</Popup>
            </CircleMarker>
          </MapContainer>
        ...
        <section className="content-sheet">
          {activeTab === "home" && <HomeScreen routeInfo={routeInfo} />}
  ...
  return (
    <>
      ...
      <div className="primary-route-card">
        <div>
          <h4>{routeInfo.from} → {routeInfo.to}</h4>
